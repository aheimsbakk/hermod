...
class TestLoadConfig:
    def test_defaults(self, tmp_path: Path) -> None:
        ...
        assert cfg.server == "wss://localhost:4430"
        assert cfg.port == 4430
        assert cfg.ttl == 3600
    ...
    def test_cli_overrides_yaml(self, tmp_path: Path) -> None:
        yaml_file = tmp_path / "config.yaml"
        yaml_file.write_text("server: wss://yaml-server:8765\n")
        cfg = load_config(
    ...
    def test_invalid_port_env_ignored(self, tmp_path: Path, monkeypatch) -> None:
        ...
        cfg = load_config(config_path=tmp_path / "missing.yaml")
        assert cfg.port == 4430  # falls back to default
...
class TestSaveConfig:
    def test_save_and_reload(self, tmp_path: Path) -> None:
        cfg = HermodConfig(server="wss://save-test:8765", ttl=1800)
        path = tmp_path / "hermod" / "config.yaml"
        save_config(cfg, path)
        assert path.exists()
        loaded = load_config(config_path=path)
        assert loaded.server == "wss://save-test:8765"
