...
```
Wraps a UDP socket and demultiplexes incoming packets into two channels: one for probe packets (first byte `0x01`) and one for QUIC packets (everything else). The returned `*packetMux` is passed to `HolePunch`, `RaceQUIC`, `DialQUIC`, and `ListenQUIC`.
...
```
Dials a QUIC connection to `peerAddr` over the muxed UDP socket. Pins the peer certificate to `peerCertHash` (64-char hex SHA-256). Kept for legacy callers; new code should use `RaceQUIC`.

```go
func ListenQUIC(mux *packetMux, cert tls.Certificate, baseTLS *tls.Config, peerCertHash string) (*quic.Listener, error)
```
Listens for incoming QUIC connections over the muxed UDP socket. Uses `cert` as the server certificate. Pins the connecting client's certificate to `peerCertHash`. Kept for legacy callers; new code should use `RaceQUIC`.
...
```
Returns a `*tls.Config` with TLS 1.3 minimum version and curve/cipher preferences from `cfg`. ALPN (`hermod-p2p`) is set separately by `RaceQUIC` (and legacy `DialQUIC` / `ListenQUIC`) in `internal/network`.
