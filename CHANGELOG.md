...
- **model:** opencode/deepseek-v4-flash-free
- **tags:** fix, ttl, timeout

### Fixed

- WebSocket idle timeout is no longer hardcoded to 120 seconds — it is driven by the `--ttl` flag (default 600s), matching user expectation (`internal/server/server.go`)
- `--ttl` flag description updated to reflect that it controls both idle timeout and channel TTL (`internal/cli/serve.go`)

### Added

- `TestServerTTLControlsWSIdleTimeout` verifies that the WebSocket idle timeout matches the configured TTL (`internal/server/server_ws_test.go`)
